...
void LOGW(char *text) {
    fprintf(stderr, "[WARNNING]: %s", text);
}

void LOGI(char *text) {
    fprintf(stdout, "[INFO]: %s", text);
}

void LOGS(char *text) {
    fprintf(stdout, "[SUCCESS]: %s", text);
}

void showup() {
    fprintf(stderr, "\t\t[OPPO SPLASH MODIFIED TOOL]\n");
    fprintf(stderr, "=---------------------------------------------------------------=\n");
    fprintf(stderr, "\tAUTHOR:\t %s \tVERSION:\t %s \n", AUTHOR, VERSION);
    fprintf(stderr, "=---------------------------------------------------------------=\n");
}
...
static PyObject *py_readinfo(PyObject *self, PyObject *args) {
    char *infile;
    if (!PyArg_ParseTuple(args, "s", &infile)) {
        Py_DECREF(args);
        Py_RETURN_NONE;
    }
    return Py_BuildValue("i", readinfo(infile));
}

static PyObject *py_unpack(PyObject *self, PyObject *args) {
    char *infile, *outdir;
    if (!PyArg_ParseTuple(args, "ss", &infile, &outdir)) {
        Py_DECREF(args);
        Py_RETURN_NONE;
    }
    return Py_BuildValue("i", unpack(infile, outdir));
}

static PyObject *py_repack(PyObject *self, PyObject *args) {
    char *infile, *outfile;
    if (!PyArg_ParseTuple(args, "ss", &infile, &outfile)) {
        Py_DECREF(args);
        Py_RETURN_NONE;
    }
    return Py_BuildValue("i", repack(infile, outfile));
}

static PyMethodDef Methods[] = {
    {"readinfo", py_readinfo, METH_VARARGS},
    {"unpack", py_unpack, METH_VARARGS},
    {"repack", py_repack, METH_VARARGS},
    {NULL, NULL},
};
...
static struct PyModuleDef cModule = {
    ...
    "splishsplash", 
    "oppo splash unpack/repack lib", 
    -1, 
    Methods
};

PyMODINIT_FUNC PyInit_splishsplash(void){ return PyModule_Create(&cModule);}
